...
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  ...
  try {
    // 2. Subida real a Cloudinary
    // Nota: Si tu función uploadToCloudinary no soporta 'onProgress', 
    // puedes dejar la simulación de progreso para que el usuario no se desespere.
    
    const response = await uploadToCloudinary(file);

    // 3. Validar la respuesta de Cloudinary
    if (!response || !response.secure_url) {
      throw new Error("Error al obtener la URL de Cloudinary");
    }

    // 4. Devolvemos la URL real que Tiptap insertará en el editor
