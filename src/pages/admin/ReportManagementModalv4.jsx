...
const ReportManagementModal = ({ report, onClose }) => {
  ...
  const isUser = !!report.reported_user;

  const targetId = isPost
    ? report.post.id
    : isComment
      ? report.comment.id
      : report.reported_user?.id;

  const targetContectType = isPost ? "post" : isComment ? "comment" : "user";

  const authorId = isPost
    ? report.post.author.id
    : isComment
      ? report.comment.author.id
      : report.reported_user?.id;

  const mediaItems = isPost ? report.post.post_media || [] : [];

  const getReportTypeUI = () => {
    if (isPost) {
      return {
  ...
  const reportTypeUI = getReportTypeUI();
  ...
  const handleBan = () => {
    if (
      window.confirm("¿Estás seguro de banear a este usuario permanentemente?")
    ) {
      ban(
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-neutral-900 rounded-t-4xl md:rounded-4xl shadow-2xl flex flex-col md:flex-row animate-in slide-in-from-bottom md:zoom-in-95 duration-200 max-h-[90vh] md:max-h-[85vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 border-b md:border-b-0 md:border-r border-gray-100 dark:border-neutral-800">
          <div className="flex justify-between items-center gap-2 mb-4">
            ...
            {/* Tipo */}
            <span
              className={`text-xs px-2 py-1 rounded-full font-medium ${reportTypeUI.className}`}
            >
              {reportTypeUI.label}
            </span>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-neutral-800/50 p-4 rounded-2xl mb-6">
            <div className="flex items-center gap-2 mb-2">
              <img
                src={
                  optimizeMedia(report.reporter.avatar, "image") ||
                  "/default-avatar.jpg"
                }
          ...
          <div className="border border-gray-200 dark:border-neutral-700 rounded-2xl p-4">
            ...
            <p className="text-base dark:text-white mb-3 whitespace-pre-line">
              ...
              {isComment && report.comment.content}
              {isUser && (
                <span className="text-gray-400 italic">
                  Reporte de usuario (sin contenido específico)
                </span>
              )}
            </p>
            ...
            {mediaItems.length > 0 && (
              <div className="mt-4 grid grid-cols-1 gap-3">
                {mediaItems.map((media, idx) => (
                  <div
                    key={idx}
                    className="relative rounded-xl overflow-hidden bg-black flex items-center justify-center min-h-[200px]"
                  >
        ...
        <div className="w-full md:w-[320px] p-6 flex flex-col gap-3 bg-white dark:bg-neutral-900 md:bg-gray-50/30 md:dark:bg-black/20 shrink-0">
          ...
          <div className="mt-auto pt-4 md:flex justify-end hidden">
            <button
              onClick={onClose}
              className="px-6 py-2 bg-gray-200 dark:bg-neutral-800 text-gray-700 dark:text-gray-300 rounded-full text-sm font-bold dark:hover:bg-gray-600 transition-colors"
            >
              Cerrar
            </button>
          </div>
