...
const ChatWindow = ({ activeChat, user, onBack, onlineUsers, isMobile }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white transition-colors duration-300">
      {/* HEADER DEL CHAT */}
      <div
        className={`flex justify-between items-center gap-5 p-4 md:p-6 bg-white/90 dark:bg-black/90 backdrop-blur-xl z-40 border-b border-gray-200 dark:border-gray-800/50 sticky top-15 shadow-sm transition-all`}
      >
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar pb-20"
      >
            ...
            return (
              <React.Fragment key={msg.id || index}>
                ...
                <div
                  ...
                  onClick={() =>!isDeleted && isMobile && isMine && setSelectedMessage(msg)}
                >
                  {/* Botón borrar Desktop */}
                  {!isDeleted && !isMobile && isMine && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <div
                    className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-[14.5px] shadow-sm transition-all ${
                      isDeleted
                        ? "bg-transparent border border-zinc-200 dark:border-zinc-800 text-zinc-400 italic"
                        : isMine
                          ? "bg-indigo-600 text-white rounded-br-none"
                          : "bg-zinc-300 dark:bg-zinc-800 dark:text-white rounded-bl-none border dark:border-zinc-700/50"
                    }`}
                  >
                   ...
                   {!isDeleted && <div
                      className={`text-[10px] mt-1.5 flex items-center gap-1 ${isMine ? "text-indigo-200 justify-end" : "text-zinc-400 justify-start"}`}
                    >
          ...
          <div className="relative w-full bg-white dark:bg-neutral-900 rounded-t-32px p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center gap-4 p-4 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-2xl"
              >
              ...
              {selectedMessage.sender_id === user.id &&(
                <button
                  ...
                  className="w-full flex items-center gap-4 p-4 text-red-500 hover:bg-red-50 rounded-2xl"
                >
              ...
              <button
                onClick={() => setSelectedMessage(null)}
                className="w-full p-4 text-zinc-500 font-bold bg-zinc-50 dark:bg-zinc-800 rounded-2xl mt-2"
              >
                Cancelar
              </button>
            </div>
