...
const HomeLayout = () => {
  ...
  useEffect(() => {
    ...
    if (errorDescription) {
      ...
      notify.error("Error de acceso: Solo se permiten correos institucionales");
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black transition-colors duration-300">
      ...
      {/* Layout Principal */}
      <div
        className={`${isMobile ? "pt-16" : "pt-10"} flex max-w-7xl mx-auto min-h-[calc(100vh-64px)]`}
      >
        {/**pt-16 */}
        ...
        <main className="flex-1 min-h-[500px] border-x border-emerald-500/10 dark:border-emerald-500/20 max-w-2xl mx-auto lg:mx-0">
          {/* min-h-[1050px]*/}
          <div className=" sm:px-0 py-0 sm:py-6">
            {/**py-4 */} {/**px-4 */}
            <Outlet />
          </div>
        </main>
